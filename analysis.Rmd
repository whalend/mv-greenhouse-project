...
By request, I'm going to evaluate Kerry's analysis by first repeating what she did using her code. I copied over all lines from the "Analysis of full data set.docx" and will be editing things in this report.
...
gh_data <- gh_data %>% 
  ...
  mutate(Light = as.factor(Light), Herbivory = as.factor(Herbivory),
         `Pop#Lat` = (1/`Pop#Lat`), Block = as.factor(Block)) %>% 
...
## Model: Date of first flowering
 ...
 the date of first flowering. The date of first flowering is ordered by day of year, so larger numbers indicate later in the calendar year.
...
These plots indicate that flowering date is earlier at higher latitudes, and that herbivory and light treatments are not siginificant.
...
# step(flowering_mod)
kable(dredge(refitML(flowering_mod)) %>% filter(delta < 7), caption = "Model comparison for ∂AICc < 7")
...
Preliminary plots of average tiller length with effects of latitude, light, and herbivory indicate a stronger effect of herbivory in the shade treatments than in the ambient light treatments. Also, the effect of herbivory in the ambient light treatments is similar in magnitude to the shade only treatment.

```{r prelim plots avg tiller length}

ggplot(gh_data, aes(Lat, avg_tiller_length, color = Light, shape = Herbivory)) +
  geom_point() +
  geom_smooth(aes(linetype = Herbivory), method = "lm", se = F) +
  scale_color_discrete(h = c(60,120)) +
  guides(linetype = guide_legend(override.aes = list(size = .7)),
...
ggplot(avg_harv_date, aes(Light, avg_seed_mass, color = pop_lat)) +
  ...
  geom_point(size = 2) +
  # geom_smooth(aes(linetype = Light), method = "lm", se = F) +
  scale_color_discrete(h = c(0,160)) +
  # guides(linetype = guide_legend(override.aes = list(size = .7)),
             # color = guide_legend(override.aes = list(linetype = 0))) +
  # ylab("Day of Year") +
  theme_bw()
...
ggplot(avg_gh_data_light, 
       aes(Light, avg_tiller_length, color = as.numeric(pop_lat))) +
  # geom_boxplot()+
  geom_path(aes(group = pop_lat)) +
  geom_point(size = 2) +
  # geom_smooth(aes(linetype = Light), method = "lm", se = F) +
...
m4<-lmer(avg_tiller_length ~ Light*Herbivory*Lat + (1|Block), data = gh_data)
...
m4.2<-lmer(log(avg_tiller_length) ~ Light*Herbivory*Lat + (1|Block), data = gh_data)
...
m4.3<-lmer(sqrt(avg_tiller_length) ~ Light*Herbivory*Lat + (1|Block), data = gh_data)
...
m4.1<-lmer(avg_tiller_length ~ Herbivory + Lat*Light + (1|Block), data = gh_data)
plot(m4.1)
summary(m4.1)


AICc(m4, m4.2, m4.3, m4.1)
BIC(m4, m4.2, m4.3, m4.1)
...
ggplot(avg_gh_data, 
       aes(Light, avg_mv_biomass, color = as.numeric(pop_lat))) +
  # geom_boxplot()+
  geom_path(aes(group = pop_lat)) +
  geom_point(size = 2) +
  facet_wrap(~Herbivory) +
...
ggplot(avg_gh_data, 
       aes(Light, avg_seed_mass, color = as.numeric(pop_lat))) +
  # geom_boxplot()+
  geom_path(aes(group = pop_lat)) +
  geom_point(size = 2) +
  facet_wrap(~Herbivory) +
...
ggplot(avg_gh_data_light, 
       aes(Light, seed_plant_ratio, color = as.numeric(pop_lat))) +
  # geom_boxplot()+
  geom_path(aes(group = pop_lat)) +
  geom_point(size = 2) +
  # geom_smooth(aes(linetype = Light), method = "lm", se = F) +
