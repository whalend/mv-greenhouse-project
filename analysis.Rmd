...
avg_gh_data <- gh_data %>% 
  group_by(pop_lat, Herbivory, Light) %>% 
  summarise(avg_harvest_date = mean(HarvestOrdDate, na.rm = T),
            avg_flower_date = mean(FlowerOrdDate, na.rm = T),
            avg_seed_mass = mean(AvgSeedMass, na.rm = T),
            avg_tiller_length = mean(avg_tiller_length, na.rm = T),
...
ggplot(avg_gh_data, 
       aes(Light, avg_flower_date, color = as.numeric(pop_lat))) +
  # geom_boxplot()+
...
bargraph.CI(pop_lat, FlowerOrdDate, group = Light, data = filter(gh_data, Herbivory == "N"), legend = T,
...
ggplot(avg_gh_data, 
       aes(Light, avg_harvest_date, color = as.numeric(pop_lat))) +
  # geom_boxplot()+
  geom_path(aes(group = pop_lat)) +
  geom_point(size = 2) +
  facet_wrap(~Herbivory) +
