...
I (WD) will be making some revisions to this as I go.
...
avg_gh_data_light <- no_herbivory %>% 
...
A mixed-effects model to examine the interaction of light and herbivory treatments with latitude of seedstock origin on the date of first flowering. The date of first flowering is ordered by day of year, so larger numbers indicate later in the calendar year.

```{r day of first flower prelim figures}

lt_flower_plot <- ggplot(avg_gh_data, 
       aes(Light, avg_flower_date, color = Lat)) +
  # geom_boxplot()+
...
herb_flower_plot <- ggplot(avg_gh_data, 
       aes(Herbivory, avg_flower_date, color = Lat)) +
  # geom_boxplot()+
  geom_path(aes(group = Lat)) +
  geom_point(size = 2) +
  facet_wrap(~Light) +
...
m1 <- lmer(FlowerOrdDate ~ Light*Herbivory*Lat + (1|Block), data=gh_data_noNA)
Anova(m1)
...
ggplot(gh_data, aes(Lat, FlowerOrdDate)) +
  geom_smooth(aes(color = Light), method = "lm", se = F) +
  geom_point(aes(color = Light)) +
  theme_classic() +
  xlab("Latitude") +
  ylab("Day of flowering")

# summary(m1)

```

Latitude of the origin population was the only signficant explanatory variable, however, a model that also included the Light treatment main effect was equally supported in AIC comparisons.
