...
gh_data$pop_database <- as.factor(gh_data$pop_database)
gh_data$Herbivory <- factor(gh_data$Herbivory,
                            levels = c("N","Y"),
                            labels = c("No defoliation", "Defoliation"))
...
avg_gh_data <- gh_data %>% 
  group_by(Lat, pop_lat, Herbivory, Light) %>% 
  summarise(
    
    avg_harvest_date = mean(HarvestOrdDate, na.rm = T),
    se_harvest_date = sd(HarvestOrdDate, na.rm = T) / sqrt(length(!is.na(HarvestOrdDate))),
    
    avg_flower_date = mean(FlowerOrdDate, na.rm = T),
    se_flower_date = sd(FlowerOrdDate, na.rm = T) / sqrt(length(!is.na(FlowerOrdDate))),
    
    avg_seed_mass = mean(AvgSeedMass, na.rm = T),
    se_seed_mass = sd(AvgSeedMass, na.rm = T) / sqrt(length(!is.na(AvgSeedMass))),
    
    avg_till_length = mean(avg_tiller_length, na.rm = T),
    se_till_length = sd(avg_tiller_length, na.rm = T) / sqrt(length(!is.na(avg_tiller_length))),
    
    avg_seeds_per_sh = mean(seeds_per_SH, na.rm = T),
    se_seeds_per_sh = sd(seeds_per_SH, na.rm = T) / sqrt(length(!is.na(seeds_per_SH))),
    
    avg_mv_biomass = mean(mv_biomass, na.rm = T),
    se_mv_biomass = sd(mv_biomass, na.rm = T) / sqrt(length(!is.na(mv_biomass))),
    
    avg_sla = mean(Avg.SLA, na.rm = T),
    se_sla = sd(Avg.SLA, na.rm = T)/sqrt(length(!is.na(Avg.SLA))),
    
    seed_plant_ratio = mean(AvgSeedMass/mv_biomass, na.rm = T),
    se_seed_plant_ratio = sd(AvgSeedMass/mv_biomass, na.rm = T) / sqrt(length(!is.na(AvgSeedMass/mv_biomass)))
    
    ) 


## Calculate plasticity index for each performance metric based on the light treatment
pl_index_data <- ungroup(avg_gh_data) %>%
  group_by(pop_lat, Herbivory) %>% 
  summarise(
    
    Senescence = (max(avg_harvest_date) - min(avg_harvest_date))/max(avg_harvest_date),
    
    Flowering = (max(avg_flower_date) - min(avg_flower_date))/max(avg_flower_date),
    
    `Seed mass` = (max(avg_seed_mass) - min(avg_seed_mass))/max(avg_seed_mass),
    
    `Seed production` = (max(avg_seeds_per_sh) - min(avg_seeds_per_sh))/max(avg_seeds_per_sh),
    
    `Tiller length` = (max(avg_till_length) - min(avg_till_length))/max(avg_till_length),
    
    `Plant mass` = 
      (max(avg_mv_biomass) - min(avg_mv_biomass))/max(avg_mv_biomass),
    
    `SLA` = (max(avg_sla) - min(avg_sla))/max(avg_sla)
         
    ) 
  
pl_index_data <- pl_index_data %>% gather(pl_attr, plasticity_index, Senescence:SLA)
  
pl_index_data$pop_lat <- as.factor(pl_index_data$pop_lat)


## Pool data across populations, i.e. group by treaments only

pooled_data <- gh_data %>% 
  group_by(Herbivory, Light) %>% 
  summarise(
...
## Exclude plants in the herbivory treatment
...
```{r plot plasticity index}

# ggplot(select(pl_index_data, pop_lat, Herbivory, mv_biomass_pl_index), 
#        aes(x = pop_lat, y = mv_biomass_pl_index)) +
...
ggplot(pl_index_data, aes(x = pop_lat, y = plasticity_index)) +
  geom_bar(aes(fill = Herbivory), stat = "identity", position = "dodge") +
  coord_flip() +
  facet_grid(pl_attr+Herbivory~.) +
  theme_bw() +
  ylab("Plasticity index") + xlab("Population") +
...
* Herbivory by fall army worm didn't reduce overhead leaf area enough (mean = 13.1%); supplemented to have total reduction of 45.5% overhead leaf area across all plants. 
...
Were all the plants started at the same time? If so, would it make sense to calculate the number days until first flowering? It should probably show the same relationship.

What is the biological significance of flowering date?

### Day of firest flowering figure
```{r flowering_prelim_fig}

ggplot(gh_data, aes(Lat, FlowerOrdDate)) +
  # geom_point(aes(shape = Herbivory)) +
  # stat_summary(fun.y = mean, geom = "line") +
  stat_summary(fun.data = mean_se, geom = "pointrange", aes(shape = Herbivory, color = Light)) +
  scale_color_grey(start = .8, end = .2) +
  geom_smooth(aes(color = Light, linetype = Herbivory), method = "lm", se = F) +
  # theme_bw() +
  theme_classic() +
...
ggplot(avg_gh_data, aes(factor(pop_lat), avg_flower_date, fill = Light)) +
  geom_bar(stat = "identity", position = "dodge", width = .7, color = "black") +
  geom_errorbar(aes(ymin = avg_flower_date - se_flower_date, ymax = avg_flower_date + se_flower_date), width = .1, position=position_dodge(width = .7)) +
  scale_fill_manual(values = c("grey97","grey35")) +
  # scale_fill_discrete(h = c(120,40),l = 70) +
  facet_wrap(~Herbivory) +
  # theme_classic() +
  theme_bw() +
  xlab("Population") + ylab("Average day of first flower") +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
        legend.title = element_blank())
# ggsave("Figures/flowering_bargraph.png", dpi = 600, width = 7.5)

```

```{r flowering_react_norm}

ggplot(avg_gh_data, aes(Light, avg_flower_date, color = Lat)) +
  # geom_boxplot()+
  ...
  # guides(linetype = guide_legend(override.aes = list(size = .7)),
             # color = guide_legend(override.aes = list(linetype = 0))) +
  ylab("Flowering day of year") +
  xlab("") +
  theme_bw() +
  theme(strip.background = element_blank(),
        panel.grid = element_blank())

ggplot(pooled_data, aes(Light, avg_flower_date)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
...
flowering_mod3 <- lmer(FlowerOrdDate ~ factor(pop_lat) + (1|Block), 
                      data = gh_data_noNA)
summary(flowering_mod3)
...
```{r senescence_barplot}

ggplot(avg_gh_data, aes(factor(pop_lat), avg_harvest_date, fill = Light)) +
  geom_bar(stat = "identity", position = "dodge", width = .7, color = "black") +
  geom_errorbar(aes(ymin = avg_harvest_date - se_harvest_date, ymax = avg_harvest_date + se_harvest_date), width = .1, position=position_dodge(width = .7)) +
  scale_fill_manual(values = c("grey97","grey35"))+
  facet_wrap(~Herbivory) +
  # theme_classic() +
  theme_bw() +
  xlab("Population") + ylab("Average day of senescence (harvest)") +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
        legend.title = element_blank())
...
```{r senescence_react_norm}

ggplot(avg_gh_data, aes(Light, avg_harvest_date, color = Lat)) +
  geom_path(aes(group = Lat)) +
  geom_point(size = 2) +
  facet_wrap(~Herbivory) +
  scale_color_continuous(low = "green", high = "brown") +
  ylab("Harvest (senescence) day of year") +
  xlab("") +
  theme_bw() +
  theme(strip.background = element_blank(),
        panel.grid = element_blank())

ggplot(pooled_data, aes(Light, avg_harvest_date)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("green", "brown")) +
  ylab("Harvest (senescence) day of year") +
...
# ggplot(avg_gh_data,
#        aes(Light, avg_harvest_date, color = (pop_lat))) +
#   # geom_boxplot()+
#   geom_path(aes(group = Lat)) +
#   geom_point(size = 2) +
#   facet_wrap(~Herbivory) +
...
senescence_mod <- lmer(HarvestOrdDate ~ Light*Herbivory*Lat + (1|Population) + (1|Block), data = gh_data_noNA)
...
senescence_mod2 <- lmer(HarvestOrdDate ~ Light*Herbivory*Lat + (1|Block), 
             data = gh_data_noNA) 

# summary(senescence_mod2@frame)
# plot(senescence_mod2)
# plot(fitted(senescence_mod2),residuals(senescence_mod2))
# qqnorm(residuals(senescence_mod2))

Anova(senescence_mod2)
step(senescence_mod2)# keep block
filter(dredge(refitML(senescence_mod2)), delta < 7)
```

### Senescence: final model
```{r senescence final model}

senescence_mod2 <- lmer(HarvestOrdDate ~ Lat + Light + (1|Block), 
                         data = gh_data_noNA)
summary(senescence_mod2)
# confint(senescence_mod2)

senescence_mod3 <- lmer(HarvestOrdDate ~ Lat + Light + (1|Population) + (1|Block),
                         data = gh_data_noNA)
summary(senescence_mod3)
...
```{r tiller_length_prelim_fig}

ggplot(gh_data, aes(Lat, avg_tiller_length, color = Light, shape = Herbivory, linetype = Herbivory)) +
  # geom_point() +
  # stat_summary(fun.y = mean, geom = "line") +
...
```{r tiller_length_react_norm}

ggplot(avg_gh_data, aes(Light, avg_till_length, color = Lat)) +
  geom_path(aes(group = Lat)) +
  geom_point(size = 2) +
  scale_color_continuous(low = "green", high = "brown") +
  facet_grid(.~Herbivory) +
...
ggplot(pooled_data, aes(Light, avg_till_length)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("green", "brown")) +
  ylab("Average tiller length (cm)") +
...
tiller_mod2 <- lmer(avg_tiller_length ~ Lat + Light + Herbivory + pop_lat + (1|Block), data = gh_data_noNA)
summary(tiller_mod2)
...
```{r mv_biomass_barplot}

ggplot(avg_gh_data, aes(factor(pop_lat), avg_mv_biomass, fill = Light)) +
  geom_bar(stat = "identity", position = "dodge", width = .7, color = "black") +
  geom_errorbar(aes(ymin = avg_mv_biomass - se_mv_biomass, ymax = avg_mv_biomass + se_mv_biomass), width = .1, position=position_dodge(width = .7)) +
  scale_fill_manual(values = c("grey97","grey35"))+
  facet_wrap(~Herbivory) +
  # theme_classic() +
  theme_bw() +
  xlab("Population") + 
  ylab("Average plant biomass (g)") +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
        legend.title = element_blank())
...
ggplot(pooled_data, aes(Light, avg_mv_biomass)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("green", "brown")) +
  ylab("Plant biomass (g)") +
...
```{r seed_production_barplot}

ggplot(avg_gh_data, aes(factor(pop_lat), avg_seeds_per_sh, fill = Light)) +
  geom_bar(stat = "identity", position = "dodge", width = .7, color = "black") +
  geom_errorbar(aes(ymin = avg_seeds_per_sh - se_seeds_per_sh, ymax = avg_seeds_per_sh + se_seeds_per_sh), width = .1, position=position_dodge(width = .7)) +
  scale_fill_manual(values = c("grey97","grey35"))+
  facet_wrap(~Herbivory) +
  # theme_classic() +
  theme_bw() +
  xlab("Population") + 
  ylab("Average seeds per seed head") +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
        legend.title = element_blank())
...
```{r seed_production_react_norm}

ggplot(avg_gh_data,
       aes(Light, avg_seeds_per_sh, color = Lat)) +
  # geom_boxplot()+
  geom_path(aes(group = Lat)) +
  geom_point(size = 2) +
  facet_wrap(~Herbivory) +
...
ggplot(pooled_data, aes(Light, avg_seeds_per_sh)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("green", "brown")) +
  ylab("Average seeds per seed head") +
...
seeds_mod3 <- lmer(seeds_per_SH ~ Lat + Herbivory + Light + Herbivory:Lat + (1|Block) + (1|pop_lat), data = gh_data_noNA)
...
```{r seed_mass_prelim_figure}

ggplot(gh_data, aes(Lat, AvgSeedMass*1000, color = Light)) +
  geom_point() +
  scale_color_discrete(h = c(50,110)) +
  geom_smooth(aes(linetype = Herbivory), method = "lm", se = F) +
  # theme_bw() +
  theme_classic() +
  ylab("Average seed mass (mg)") +
  xlab("Latitude")
...
ggplot(avg_gh_data, aes(factor(pop_lat), avg_seed_mass*1000, fill = Light)) +
  geom_bar(stat = "identity", position = "dodge", width = .7, color = "black") +
  geom_errorbar(aes(ymin = avg_seed_mass*1000 - se_seed_mass*1000, ymax = avg_seed_mass*1000 + se_seed_mass*1000), width = .1, position=position_dodge(width = .7)) +
  scale_fill_manual(values = c("grey97","grey35"))+
  facet_wrap(~Herbivory) +
  # theme_classic() +
  theme_bw() +
...
```{r seed_mass_react_norm}

ggplot(avg_gh_data,
       aes(Light, avg_seed_mass*1000, color = Lat)) +
  # geom_boxplot()+
  geom_path(aes(group = Lat)) +
  geom_point(size = 2) +
  facet_wrap(~Herbivory) +
...
ggplot(pooled_data, aes(Light, avg_seed_mass*1000)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("green", "brown")) +
  ylab("Average seed mass (mg)") +
...
seed_mass_mod3 <- lmer(AvgSeedMass ~ FlowerOrdDate + Lat + Light + Light:Lat + (1|Block), na.action = "na.omit", data = gh_data_noNA)
summary(seed_mass_mod3)
...
ggplot(avg_gh_data, aes(factor(pop_lat), avg_sla, fill = Light)) +
  geom_bar(stat = "identity", position = "dodge", width = .7, color = "black") +
  geom_errorbar(aes(ymin = avg_sla - se_sla, ymax = avg_sla + se_sla), width = .1, position=position_dodge(width = .7)) +
  scale_fill_manual(values = c("grey97","grey35"))+
  facet_wrap(~Herbivory) +
  # theme_classic() +
  theme_bw() +
...
ggplot(pooled_data, aes(Light, avg_sla)) +
  geom_path(aes(linetype = Herbivory, group = Herbivory)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("green", "brown")) +
  ylab("Average SLA") +
