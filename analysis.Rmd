...
author: "K.B. Stricker, W.W. Dillon, D.T. Hiatt, A.E. Kendig, S.L. Flory"
...
Seeds were sourced from 12 populations spanning ~6 degrees of latitude (32.58º to 38.97º) in INSERT CONTINENT/COUNTRY and then grown in a common greenhouse experiment. There were 2-3 seeds per plot and they were exposed to either shade or ambient lighting for the duration of the experiment. The shade treatment decreased light by ~90%. 
...
I'm evaluating Kerry's analysis by first repeating what she did using her code and embedding figures.
...
```{r anova date of first flower, results='asis'}
...
plot(residuals(flowering_mod),fitted(flowering_mod))

# step(flowering_mod)
kable(dredge(refitML(flowering_mod)) %>% filter(delta < 7), caption = "Models where ∆AICc < 7 compared to best model")
# plot(gh_data$Lat, gh_data$FlowerOrdDate)
# FlowerReg <- lm(FlowerOrdDate~Lat, data = gh_data)
# abline(FlowerReg)

# summary(flowering_mod)
summary(update(flowering_mod, . ~ Lat + Light + (1|Block) ))

confint(update(flowering_mod, . ~ Lat + Light + (1|Block) ))

```

Latitude of the origin population was the only signficant explanatory variable for the flowering date response, however, a model that also included the *Light* treatment main effect was equally supported in AIC comparisons. In general, flowering date was earlier for populations from from higher latitudes and in the shade treatments.
...
plot(residuals(senescence_mod), fitted(senescence_mod))
...
summary(update(senescence_mod.2, . ~ Lat + Light + (1|Block)))
...
# tiller_mod.2<-lmer(log(avg_tiller_length) ~ Light*Herbivory*Lat + (1|Block), data = gh_data)
...
# tiller_mod.3<-lmer(sqrt(avg_tiller_length) ~ Light*Herbivory*Lat + (1|Block), data = gh_data)
# qqnorm(residuals(tiller_mod.3))
# 
# # plot(fitted(tiller_mod.3),residuals(tiller_mod.3))
...
# 
# 
# AICc(tiller_mod, tiller_mod.2, tiller_mod.3, tiller_mod.1)
# BIC(tiller_mod, tiller_mod.2, tiller_mod.3, tiller_mod.1)
# step(tiller_mod.3)

summary(update(tiller_mod, . ~ Lat + Light + Herbivory + (1|Block)))
...
Anova(mv_biomass_mod)
## Indicates significant main effects, and Light:Herbivory interaction
...
plot(update(mv_biomass_mod, log1p(mv_biomass) ~ .), main = "ln+1 transformed")
plot(update(mv_biomass_mod, sqrt(mv_biomass) ~ .), main = "square-rooted")
## Log transformation results in better residuals

mv_biomass_mod2 <- update(mv_biomass_mod, log1p(mv_biomass) ~ .)
...
mv_biomass_mod2 <- update(mv_biomass_mod, . ~ Lat + Light + Herbivory + (1|Block))
qqnorm(residuals(mv_biomass_mod2))
...
3. What is the biological intepretation?
...
There are `r n_distinct(filter(gh_data, is.na(seeds_per_SH)))` missing values (`NA`) for seeds/seed-head - is this because no seed heads were produced? It also raises the questions of whether these should be treated as true zeroes, i.e. zero seeds were produced, or excluding them as "missing" is the appropriate approach.

Using zeroes or excluding these observations as missing has no effect on the overall relationship, but it does change the coefficient estimate for the herbivory effect by seven. I'm don't think this is a biologically meaningful difference, but it may be more biologically accurate if it is reasonable to think that the light or herbivory treatments may have prevented seeds from being produced. 
...
dredge(refitML(seeds_mod2))

# summary(update(seeds_mod1, . ~ Lat + Herbivory + Light + Herbivory:Lat + (1|Block)))
summary(update(seeds_mod2, . ~ Lat + Herbivory + Light + Herbivory:Lat + (1|Block)))
...
Is it interesting that average seed mass was greater in the shade? Shade results in fewer but also more massive seeds? 
...
## Model: Seed mass/plant mass ratio
...
Specific leaf area was greater in the shade, and slightly increased by herbivory treatments.
...
> **KS**: So basically, what's going on here is that there is a positive relationship between latitude and herbivory and latitude when plants grown under ambient light and herbivory. Otherwise the lines show a negative/no relationship. Of course, seeds/seed head number is reduced overall when light reduced.
